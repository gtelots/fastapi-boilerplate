...
def get_sync_postgres_dsn() -> str:
    ...
    return app_settings.get_postgres_dsn("psycopg2").replace("%", "%%")
...
def assert_dev_or_testing() -> None:
    if not (app_settings.is_development() or app_settings.is_testing()):
        raise RuntimeError(f"DANGER! You cannot run this script in {app_settings.ENV}!")
