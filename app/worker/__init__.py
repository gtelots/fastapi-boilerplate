...
from app.logfire import instrument_httpx
...
class MaxRetriesMiddleware(dramatiq.Middleware):
    ...
    def before_process_message(
        ...
        max_retries = message.options.get(
            "max_retries", actor.options.get("max_retries", app_settings.WORKER_MAX_RETRIES)
        )
...
class LogfireMiddleware(dramatiq.Middleware):
    ...
    def before_process_message(
        ...
        if actor_name in app_settings.LOGFIRE_IGNORED_ACTORS:
...
broker = RedisBroker(
    connection_pool=redis.ConnectionPool.from_url(
        ...
        client_name=f"{app_settings.ENV.value}.worker.dramatiq",
    ),
