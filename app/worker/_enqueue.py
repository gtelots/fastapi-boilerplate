...
_job_queue_manager: contextvars.ContextVar["JobQueueManager | None"] = (
    contextvars.ContextVar("app.job_queue_manager")
)
...
class JobQueueManager:
    ...
    def enqueue_job(
        ...
        log.debug("app.worker.job_enqueued", actor=actor)
    ...
    async def flush(self, broker: dramatiq.Broker, redis: Redis) -> None:
        ...
        for actor_name, encoded_message in all_messages:
            log.debug(
                "app.worker.job_flushed", actor=actor_name, message=encoded_message
            )
